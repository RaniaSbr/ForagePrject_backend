...
public class PhaseStatus {
    ...
    public String getEtatCout() { return etatCout; }
    public String getEtatDelai() { return etatDelai; }
    public String getCouleurCout() { return couleurCout; }
    public String getCouleurDelai() { return couleurDelai; }

    // --- Méthodes internes ---

    private String calculerEtat(double valeurReelle, double valeurPrevue) {
        if (valeurPrevue == 0) return "NORMAL"; // Évite division par zéro
        if (valeurReelle >= valeurPrevue) return "DANGER";
        if (valeurReelle >= 0.8 * valeurPrevue) return "RISQUE";
        return "NORMAL";
    }
